...
| REST API                  | Hono HTTP adapter: create/read/vread/update(+create-on-update)/delete, ETag/Location/versioning, instance + type history Bundles, OperationOutcome errors (search endpoints next) | ✅ Built ([ADR 0005](docs/adr/0005-hono-http.md))                   |
...
| AuthN/AuthZ               | JWT + RBAC + SMART-style scopes (`patient/Observation.read`, `user/*.read`)                                                                                                       | 🚧 Planned ([ADR 0008](docs/adr/0008-jwt-and-smart-scopes.md))      |
